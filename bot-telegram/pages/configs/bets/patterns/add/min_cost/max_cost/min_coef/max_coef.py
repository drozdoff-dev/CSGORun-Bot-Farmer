...
def call_page(app_path,command):
    ...
    if(create_coef_max >= 1.01):
        if(create_coef_max >= info_user_tg.storage['args']['pattern_bet']['coef_min']):
            info_user_tg.storage['args']['pattern_bet']['coef_max'] = create_coef_max
            configs.storage['auto_bet']['Pattern_Bet']['patterns'][info_user_tg.storage['args']['pattern_bet']['pattern']] = {
                "coef": {
                    "min": info_user_tg.storage['args']['pattern_bet']['coef_min'],
                    "max": info_user_tg.storage['args']['pattern_bet']['coef_max']
                },
                "cost": {
                    "min": info_user_tg.storage['args']['pattern_bet']['cost_min'],
                    "max": info_user_tg.storage['args']['pattern_bet']['cost_max']
                }
            }
