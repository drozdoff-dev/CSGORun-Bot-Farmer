    ...
    if(change_max_coef >= 0.01):
        for (i,pattern_bet) in zip(range(len(configs.storage['auto_bet']['Pattern_Bet']['patterns'])),configs.storage['auto_bet']['Pattern_Bet']['patterns']):
            if(i == number_pattern_bet-1):
                if(change_max_coef >= configs.storage['auto_bet']['Pattern_Bet']['patterns'][pattern_bet]['coef']['min']):
                    info_user_tg.storage['args']['pattern_bet']['last_command'] = ''
                    info_user_tg.storage['active_page']['configs']['bets']['patterns']['edit']['change'] = {}
                    init_classes.bot_tg.send_message(f"Значение максимальной ставки у ставки №{number_pattern_bet} изменено с {configs.storage['auto_bet']['Pattern_Bet']['patterns'][pattern_bet]['coef']['max']} на {change_max_coef}")
                    configs.storage['auto_bet']['Pattern_Bet']['patterns'][pattern_bet]['coef']['max'] = change_max_coef
                    text = f''
                    ...
                    text = f"Максимальное значение коэффициента должно быть больше или равно минимальному - {configs.storage['auto_bet']['Pattern_Bet']['patterns'][pattern_bet]['coef']['min']}"
                    return init_classes.bot_tg.send_message(text,parse_mode="Markdown")
    else:
